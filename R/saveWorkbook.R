...
#' 
#' @note If the \code{filePath} does not end with ``.xlsx'' then the fiile extension will be set to ``.xlsx''. If \code{file,path} is omitted then the script file path will be used, but that extension substiuted. But an error it thrown if \code{filePath} is omitted in interactive mode.
#'
#' @examples
#'
#' book <- createWorkbook()
...
#'
#' @export
saveWorkbook <- function (workbook = getWorkbook(), filePath = thisFile(), title = NA, subject = NA, creator = Sys.getenv("USERNAME"), start = FALSE) {
	#http://thinktibits.blogspot.com/2014/07/read-write-metadata-excel-poi-example.html
	if(tolower(tools::file_ext(filePath)) != "xlsx") filePath <- paste(tools::file_path_sans_ext(filePath), "xlsx", sep=".")
